...
public class Button : MonoBehaviour {
    ...
    Button.instance = this; //싱글톤 부여
        ...
        for(int i=0;i<10;i++) {
        buttontemp[i] = GameObject.Find("Button" + i.ToString()); //Button이라는 이름의 오브젝트들을 찾아서 배열에 처넣음
        }
    }
    public void ChangeImageToDiary(int value) {
        buttontemp[value].GetComponent<Image>().sprite=diaryimage.sprite; //배열에 처넣은 오브젝트의 이미지 컴포넌트의 스프라이트를 선언한 이미지의 스프라이트로 바꿈
    }
